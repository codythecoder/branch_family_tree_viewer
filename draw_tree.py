...
class Node(pygame.sprite.Sprite):
    ...
    def move(self, clump: list['Node']):
        positions = []
        for n in clump:
            positions.append(n.rect.centerx)
        clump_centre = sum(positions)/len(positions)
        if self.rect.centerx < clump_centre:
            self.pos[0] -= 20
        elif self.rect.centerx < clump_centre:
            self.pos[0] += 20
        else:
            self.pos[0] += randrange(-20, 21)

        
        


def sort_people(path: tuple[Relation]):
    value = 0
    height = 0
    prev = None
    ...
    for p in path:
        ...
        if p == Relation.father:
            ...
            if prev == Relation.father:
                value += 2**height
            elif prev == Relation.mother:
                value -= 2**height
        prev = p
...
if __name__ == '__main__':
    ...
    def rand_path(i):
        """generate a random path upwards"""
        path = []
        for _ in range(randrange(i)):
            if randrange(2):
                path.append(Relation.child)
            elif randrange(2):
                path.append(Relation.mother)
            else:
                path.append(Relation.father)
        return path
    ...
    for _ in range(10_000):
        ...
        # make sure the father's side is further left than their mothers side
        # print(
        #     f'{sort_people(path + [Relation.father] + father):.5f}',
        #     f'{sort_people(path + [Relation.mother] + mother):.5f}',
        # )
        assert (
            sort_people(path + [Relation.father] + father)
            <
            sort_people(path + [Relation.mother] + mother)
        )

        # make sure the father's side is on the left
        assert (
            sort_people(path + [Relation.father] + father)
            <=
            sort_people(path)
        ...
        # make sure the mother's side is on the right
        assert (
            sort_people(path)
            <=
            sort_people(path + [Relation.mother] + mother)
        ...
        # assert children between parents
        assert (
            sort_people(path + [Relation.father])
            <=
            sort_people(path + [Relation.father, Relation.child])
        )
        assert (
            sort_people(path + [Relation.mother, Relation.child])
            <=
            sort_people(path + [Relation.mother])
        )

        # assert children same
        assert (
            sort_people(path + [Relation.mother, Relation.child])
            ==
            sort_people(path)
